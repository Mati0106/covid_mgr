...
# model = load_model('germany_reco_final.h5')
...
initial_results = pd.DataFrame(
    columns={'country', 'date', 'dependent_variable', 'reco_actual_initial', 'reco_predict_initial'},
...
initial_results['reco_actual_initial'] = country_data[dep_variable].values
initial_results['reco_predict_initial'] = train_data

initial_results.to_csv(path.joinpath('GER_reco_initial.csv'), index=False)
...
final_results = pd.DataFrame(
    columns={'country', 'date', 'dependent_variable', 'reco_actual_final', 'reco_predict_final'},
    index=range(0, len(times)))
final_results['country'] = 'Germany'
final_results['date'] = times
final_results['dependent_variable'] = dep_variable
final_results['reco_actual_final'] = country_data[dep_variable]
final_results['reco_predict_final'] = train_data

final_results.to_csv(path.joinpath('GER_reco_final.csv'), index=False)

# uncomment to save a model
# model.save('germany_reco_final.h5')
