...
# model = load_model('ger_deaths_final.h5')

# choose dependent variable
dep_variable = 'Deaths'
country_data = total_df[(total_df['Country/Region'] == str('Germany'))].sort_values('Confirmed').reset_index(
...
for i in range(0, 31):
    ...
    if i == -1:
        model.fit(X_train, y_train, batch_size=64, epochs=400, verbose=2, shuffle=True
                  , callbacks=[es],
                  validation_data=(X_test, y_test))  # len(new_data)-n_steps #validation_data=(X_test, y_test)
...
ax.set(xlabel="Date",
       ylabel=dep_variable,
       title="Forecast of " + dep_variable + " in Germany")
...
initial_results = pd.DataFrame(
    columns={'country', 'date', 'dependent_variable', 'deaths_actual_initial', 'deaths_predict_initial'},
    index=range(0, len(times)))
initial_results['country'] = 'Germany'
initial_results['date'] = times
initial_results['dependent_variable'] = dep_variable
initial_results['deaths_actual_initial'] = country_data[dep_variable].values
initial_results['deaths_predict_initial'] = train_data

initial_results.to_csv(path.joinpath('GER_deaths_initial.csv'), index=False)
...
final_results = pd.DataFrame(
    columns={'country', 'date', 'dependent_variable', 'deaths_actual_final', 'deaths_predict_final'},
    index=range(0, len(times)))
final_results['country'] = 'Germany'
final_results['date'] = times
final_results['dependent_variable'] = dep_variable
final_results['deaths_actual_final'] = country_data[dep_variable]
final_results['deaths_predict_final'] = train_data

final_results.to_csv(path.joinpath('GER_deaths_final.csv'), index=False)

# uncomment to save a model
# model.save('germany_deaths_final.h5')
